...
float4 frag(VertexOutput i) : COLOR {
    ...
    // テクスチャの合成
    float blend = float(saturate(abs(1 - (clamp(distance(i.posWorld.xyz, _WorldSpaceCameraPos), _FadeStartDepth, _FadeEndDepth) - _FadeStartDepth) / (_FadeEndDepth - _FadeStartDepth) - _FadeDepthInvert)));
