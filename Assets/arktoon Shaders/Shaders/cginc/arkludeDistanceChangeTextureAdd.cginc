...
float4 frag(VertexOutput i) : COLOR {
    ...
    // テクスチャの合成
    float Blend = float(abs(1 - (clamp(distance(i.posWorld.xyz, _WorldSpaceCameraPos), _ChangeTextureStartDepth, _ChangeTextureEndDepth) - _ChangeTextureStartDepth) / (_ChangeTextureEndDepth - _ChangeTextureStartDepth) - _VisibilityInversion));
    float4 _MainTex_var = UNITY_SAMPLE_TEX2D(REF_MAINTEX, TRANSFORM_TEX(i.uv0, REF_MAINTEX));
    float4 _SubTex_var = UNITY_SAMPLE_TEX2D(REF_SUBTEX, TRANSFORM_TEX(i.uv0, REF_SUBTEX));
    _MainTex_var.rgb = lerp(_MainTex_var.rgb, _SubTex_var.rgb, Blend);
